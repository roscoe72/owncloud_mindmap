...
mindmaps.ToolBarPresenter = function(eventBus, commandRegistry, view,
  ...
  // clipboard buttons
  var clipboardCommands = [ mindmaps.CopyNodeCommand, mindmaps.CutNodeCommand, mindmaps.PasteNodeCommand ];
  var clipboardButtons = commandsToButtons(clipboardCommands);
  view.addButtonGroup(clipboardButtons, view.alignLeft);

  // file menu
  var fileMenu = new mindmaps.ToolBarMenu("Mindmap", "ui-icon-document");
  var fileCommands = [ mindmaps.SaveAsPNGCommand, mindmaps.PrintCommand ];
  var fileButtons = commandsToButtons(fileCommands);
  fileMenu.add(fileButtons);
  view.addMenu(fileMenu);

  // save button
  view.addButton(commandToButton(mindmaps.SaveAsJSONCommand), view.alignRight);
