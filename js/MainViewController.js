...
mindmaps.MainViewController = function(eventBus, mindmapModel, commandRegistry) {
  ...
  this.go = function() {
    ...
    var inspectorPresenter = new mindmaps.InspectorPresenter(eventBus, mindmapModel, inspectorView);
    inspectorPresenter.go();

    var inspectorPanel = fpf.create(lang['windowstyletitle'], inspectorView.getContent());
    inspectorPanel.show();
    ...
    var navigatorPanel = fpf.create(lang['windownavigatortitle'], naviView.getContent());
    navigatorPanel.show();
    statusbarPresenter.addEntry(navigatorPanel);

/*
    $('#windowstylefontsize').html(lang['windowstylefontsize']);
    $('#windowstylefontstyle').html(lang['windowstylefontstyle']);
    $('#windowstylefontcolor').html(lang['windowstylefontcolor']);
    $('#windowstylebranchcolor').html(lang['windowstylebranchcolor']);
    $('#windowstyleinherit').html(lang['windowstyleinherit']);
    $('#printoption').html(lang['printoption']);
    $('#inspector-button-branch-color-children').attr('value', lang['windowstyleinherit']);
    $('#inspector-button-branch-color-children').attr('title', lang['windowstyleinheritD']);
*/
  };
